...
const AllPlayers = () => {
  ...
  const handleViewPicks = (playerId: string, username: string) => {
    navigate(`/player/${playerId}`, { state: { username } });
  };
                ...
                <Table>
                  ...
                  <TableBody>
                    {players.map((player, index) => (
                      <TableRow key={player.id} className="border-purple-800/30 hover:bg-purple-800/10">
                        ...
                        <TableCell>
                          <Button
                            onClick={() => handleViewPicks(player.id, player.username)}
